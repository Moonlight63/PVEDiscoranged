#!/bin/bash
# https://github.com/Moonlight63/PVEDiscoranged
...
REPO=${REPO:-"Moonlight63/PVEDiscoranged"}
DEFAULT_TAG="master"
TAG=${TAG:-$DEFAULT_TAG}
...
function checkSupported {   
    ...
    if [ -z "$SUPPORTED" ]; then 
        ...
        if ! (printf '%s\n' "${SUPPORTEDARR[@]}" | grep -q -P "$PVEVersionMajor"); then
            ...
            echo -e "If you do run into any issues on >newer< versions, please consider opening an issue at https://github.com/Moonlight63/PVEDiscoranged/issues.${REG}"
        fi
    fi
}
...
function usage {
    ...
    #    echo -e "  utility-update       Update this utility\n" (to be implemented)
        ...
        echo -e "Report issues at: <https://github.com/Moonlight63/PVEDiscoranged/issues>"
    fi
}
